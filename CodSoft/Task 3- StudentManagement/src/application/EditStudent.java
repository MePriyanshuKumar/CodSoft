...
public class EditStudent implements Initializable {
	...
	public void GetStudentInfields() {
		Student student = StudentDao.getStudentByID(DashboardManage.mySession, DashboardManage.studentId);
		System.out.println(student);
		AdminField.setText(String.valueOf(student.getAdmission_no()));
		nameField.setText(student.getName());
		guardianField.setText(student.getGuardian_name());
		...
		yearBox.getSelectionModel().select(student.getYear());
	}

	public void EditTheStudent() {
		int a = checkDetails();
		if (a == 1) {
			return;
		}
		...
		student.setAdmission_no(DashboardManage.studentId);
		System.out.println(student);
        int i =AlertOptions.ConfirmBox("Confirm to proceed!");
        if(i==1)
        {
		String result = StudentDao.UpdateStudent(DashboardManage.mySession, student);
		Labelnfo.setText(result);
        }
